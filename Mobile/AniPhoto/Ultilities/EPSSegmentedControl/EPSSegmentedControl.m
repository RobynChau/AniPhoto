...
#import "EPSGradientView.h"
#import "UIView+EPSRemoveConstraints.h"

@interface EPSSegmentedControl() <UIScrollViewDelegate, UIGestureRecognizerDelegate>
...
@property (nonatomic, strong) EPSGradientView *gradientView;
...
@implementation EPSSegmentedControl
...
- (void)initDefaults {
    ...
    self.transitionStyle = EPSSegmentedControlTransitionStyleFade;
    self.shapeStyle = EPSSegmentedControlShapeStyleLiquid;
...
- (void)initGradientView {
    ...
    self.gradientView = [[EPSGradientView alloc] initWithFrame:CGRectZero];
...
- (void)setupGradientViewWithCount:(NSInteger)count {
    ...
    ((CAGradientLayer *)self.gradientView.layer).startPoint = CGPointMake(0, 0.5);
    ((CAGradientLayer *)self.gradientView.layer).endPoint = CGPointMake(1, 0.5);
}
- (void)setupColorsArray:(NSMutableArray *)colors withCount:(NSInteger)count {
    for (NSInteger index = -1; index <= count; index++) {
        NSArray <UIColor *> *stateColors;
        if (index == -1) {
            stateColors = [self gradientColorForBounce:EPSSegmentedControlBounceLeft];
        }
        if (index >= 0 && index < count) {
            stateColors = [self gradientColorForStateAtIndex:index];
        }
        if (index == count) {
            stateColors = [self gradientColorForBounce:EPSSegmentedControlBounceRight];
        }
...
- (NSArray <UIColor *> *)gradientColorForBounce:(EPSSegmentedControlBounce)bounce {
...
- (void)removeOldConstraints {
    [self eps_removeConstraintsFromSubTree:[NSSet setWithArray:self.addedConstraintsToRemove]];
...
#pragma mark - Setters
- (void)setDataSource:(id<EPSSegmentedControlDataSource>)dataSource {
...
- (void)setTransitionStyle:(EPSSegmentedControlTransitionStyle)transitionStyle {
...
- (CGFloat)gradientLocationForBounce:(EPSSegmentedControlBounce)bounce colorIndex:(NSInteger)index colorsCount:(NSInteger)colorsCount statesCount:(NSInteger)statesCount {
    CGFloat totalCount = statesCount + 2 + (statesCount + 1) * self.gradientBackVelocity;
    switch (bounce) {
        case EPSSegmentedControlBounceLeft:
            return index * 1.0 / (colorsCount - 1) / totalCount;
            break;
            
        case EPSSegmentedControlBounceRight:
...
- (CGPathRef)pathForSelectorViewFromPercentage:(CGFloat)percent {
    switch (self.shapeStyle) {
        case EPSSegmentedControlShapeStyleRoundedRect:
            return [self roundedRectPathForSelectorViewFromPercentage:percent].CGPath;
            
        case EPSSegmentedControlShapeStyleLiquid:
