//
//  EPSClipImageViewController.swift
//  AniPhoto
//
//  Created by PhatCH on 2020/8/27.
//

import UIKit

extension EPSClipImageViewController {
    enum ClipPanEdge {
        case none
        case top
        case bottom
...
class EPSClipImageViewController: UIViewController {
    ...
    private lazy var bottomToolLineView: UIView = {
        let view = UIView()
        view.backgroundColor = .eps.rgba(240, 240, 240)
        return view
    }()
    
    private lazy var cancelBtn: EPSEnlargeButton = {
        let btn = EPSEnlargeButton(type: .custom)
        btn.setImage(.eps.getImage("zl_close"), for: .normal)
    ...
    private lazy var revertBtn: EPSEnlargeButton = {
        let btn = EPSEnlargeButton(type: .custom)
    ...
    lazy var doneBtn: EPSEnlargeButton = {
        let btn = EPSEnlargeButton(type: .custom)
        btn.setImage(.eps.getImage("zl_right"), for: .normal)
        btn.adjustsImageWhenHighlighted = false
        btn.enlargeInset = 20
    ...
    private lazy var rotateBtn: EPSEnlargeButton = {
        let btn = EPSEnlargeButton(type: .custom)
        btn.setImage(.eps.getImage("zl_rotateimage"), for: .normal)
        btn.adjustsImageWhenHighlighted = false
        btn.enlargeInset = 20
    ...
    private lazy var clipRatioColView: UICollectionView = {
        ...
        layout.itemSize = EPSClipImageViewController.clipRatioItemSize
        ...
        view.alpha = 0
        view.showsHorizontalScrollIndicator = false
        ZLImageClipRatioCell.eps.register(view)
        return view
    }()
    
    private var shouldLayout = true
    
    private var panEdge: EPSClipImageViewController.ClipPanEdge = .none
    ...
    deinit {
        eps_debugPrint("EPSClipImageViewController deinit")
        cleanTimer()
    }
    
    init(image: UIImage, status: EPSClipStatus) {
        originalImage = image
        let configuration = EPSPhotoConfiguration.default().editImageConfiguration
        ...
        if angle == -90 {
            editImage = image.eps.rotate(orientation: .left)
        } else if angle == -180 {
            editImage = image.eps.rotate(orientation: .down)
        } else if angle == -270 {
            editImage = image.eps.rotate(orientation: .right)
        } else {
            editImage = image
        }
        ...
        if let ratio = status.ratio {
            ...
            firstEnter = true
            selectedRatio = EPSPhotoConfiguration.default().editImageConfiguration.clipRatios.first!
        }
    ...
    override func viewDidAppear(_ animated: Bool) {
        ...
        if presentingViewController is EPSImageEditorViewController {
    ...
    override func viewDidLayoutSubviews() {
        ...
        bottomToolView.frame = CGRect(x: 0, y: view.bounds.height - EPSClipImageViewController.bottomToolViewH, width: view.bounds.width, height: EPSClipImageViewController.bottomToolViewH)
        ...
        let toolBtnY = (EPSClipImageViewController.bottomToolViewH - toolBtnH) / 2 - 10
        cancelBtn.frame = CGRect(x: 30, y: toolBtnY, width: toolBtnH, height: toolBtnH)
        let revertBtnW = localLanguageTextValue(.revert).eps.boundingRect(font: EPSLayout.bottomToolTitleFont, limitSize: CGSize(width: CGFloat.greatestFiniteMagnitude, height: toolBtnH)).width + 20
        ...
        let ratioColViewY = bottomToolView.frame.minY - EPSClipImageViewController.clipRatioItemSize.height - 5
        rotateBtn.frame = CGRect(x: 30, y: ratioColViewY + (EPSClipImageViewController.clipRatioItemSize.height - 25) / 2, width: 25, height: 25)
    ...
    private func generateThumbnailImage() {
        ...
        thumbnailImage = editImage.eps.resize_vI(size)
    }
    ...
    private func calculateMaxClipFrame() -> CGRect {
        ...
        rect.size.height = UIScreen.main.bounds.height - insets.top - EPSClipImageViewController.bottomToolViewH - EPSClipImageViewController.clipRatioItemSize.height - 25
        return rect
    }
...
//        self.mainScrollView.contentSize = size
    ...
    @objc private func doneBtnClick() {
        ...
        if presentingViewController is EPSCustomCamera {
    ...
    @objc private func rotateBtnClick() {
        ...
        if selectedRatio.whRatio == 0 || selectedRatio.whRatio == 1 {
            ...
            // 旋转图片
            editImage = editImage.eps.rotate(orientation: .left)
...
//            editRect = CGRect(x: editImage.size.width - rect.maxY, y: rect.minX, width: rect.height, height: rect.width)
    ...
    private func calculatePanEdge(at point: CGPoint) -> EPSClipImageViewController.ClipPanEdge {
...
//                }
    ...
    private func startTimer() {
        cleanTimer()
        // TODO: 换target写法
        resetTimer = Timer.scheduledTimer(timeInterval: 0.8, target: EPSWeakProxy(target: self), selector: #selector(endEditing), userInfo: nil, repeats: false)
    ...
    private func clipImage() -> (clipImage: UIImage, editRect: CGRect) {
        ...
        let clipImage = editImage.eps.clipImage(angle: 0, editRect: frame, isCircle: selectedRatio.isCircle)
...
extension EPSClipImageViewController: UIGestureRecognizerDelegate {
...
extension EPSClipImageViewController: UICollectionViewDataSource, UICollectionViewDelegate {
    func collectionView(_ collectionView: UICollectionView, numberOfItemsInSection section: Int) -> Int {
        return clipRatios.count
    }
    
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        let cell = collectionView.dequeueReusableCell(withReuseIdentifier: ZLImageClipRatioCell.eps.identifier, for: indexPath) as! ZLImageClipRatioCell
        
        let ratio = clipRatios[indexPath.row]
        cell.configureCell(image: thumbnailImage ?? editImage, ratio: ratio)
        
        if ratio == selectedRatio {
            cell.titleLabel.textColor = .eps.imageEditorToolTitleTintColor
        } else {
            cell.titleLabel.textColor = .eps.imageEditorToolTitleNormalColor
        }
...
extension EPSClipImageViewController: UIScrollViewDelegate {
...
extension EPSClipImageViewController: UIViewControllerTransitioningDelegate {
    func animationController(forDismissed dismissed: UIViewController) -> UIViewControllerAnimatedTransitioning? {
        return EPSClipImageDismissAnimatedTransition()
    }
}
