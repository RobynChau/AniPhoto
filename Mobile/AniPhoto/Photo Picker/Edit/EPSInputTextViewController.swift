//
//  EPSInputTextViewController.swift
//  AniPhoto
//
...
class EPSInputTextViewController: UIViewController {
    ...
    private var font: UIFont = .boldSystemFont(ofSize: EPSTextStickerView.fontSize)
    ...
    private lazy var bgImageView: UIImageView = {
        let view = UIImageView(image: image?.eps.blurImage(level: 4))
    ...
    private lazy var textView: UITextView = {
        ...
        textView.tintColor = .eps.bottomToolViewBtnNormalBgColor
    ...
    private lazy var toolView = UIView(frame: CGRect(
        x: 0,
        y: view.eps.height - Self.toolViewHeight,
        width: view.eps.width,
    ...
    private lazy var collectionView: UICollectionView = {
        ...
        ZLDrawColorCell.eps.register(collectionView)
    ...
    deinit {
        eps_debugPrint("EPSInputTextViewController deinit")
    }
    ...
    init(image: UIImage?, text: String? = nil, textColor: UIColor? = nil, font: UIFont? = nil, style: ZLInputTextStyle = .normal) {
        ...
        self.text = text ?? ""
        if let font = font {
            self.font = font.withSize(EPSTextStickerView.fontSize)
        }
    ...
    override func viewDidLayoutSubviews() {
        ...
        let cancelBtnW = localLanguageTextValue(.cancel).eps.boundingRect(font: EPSLayout.bottomToolTitleFont, limitSize: CGSize(width: .greatestFiniteMagnitude, height: EPSLayout.bottomToolBtnH)).width + 20
        cancelBtn.frame = CGRect(x: 15, y: btnY, width: cancelBtnW, height: EPSLayout.bottomToolBtnH)
        
        let doneBtnW = (doneBtn.currentTitle ?? "")
            .eps.boundingRect(
                font: EPSLayout.bottomToolTitleFont,
                limitSize: CGSize(width: .greatestFiniteMagnitude, height: EPSLayout.bottomToolBtnH)
            ).width + 20
        doneBtn.frame = CGRect(x: view.eps.width - 20 - doneBtnW, y: btnY, width: doneBtnW, height: EPSLayout.bottomToolBtnH)
        
        textView.frame = CGRect(x: 10, y: doneBtn.eps.bottom + 30, width: view.eps.width - 20, height: 200)
        ...
        collectionView.frame = CGRect(
            x: textStyleBtn.eps.right + 5,
            y: 0,
            width: view.eps.width - textStyleBtn.eps.right - 5 - 24,
            height: Self.toolViewHeight
        )
        
        if let index = EPSPhotoConfiguration.default().editImageConfiguration.textStickerTextColors.firstIndex(where: { $0 == self.currentColor }) {
        ...
        if !textView.text.isEmpty {
            for subview in textView.subviews {
                if NSStringFromClass(subview.classForCoder) == "_UITextContainerView" {
                    ...
                    image = UIGraphicsImageRenderer.eps.renderImage(size: size) { context in
                        if textStyle == .bg {
    ...
    @objc private func keyboardWillShow(_ notify: Notification) {
        ...
        let toolViewFrame = CGRect(
            x: 0,
            y: view.eps.height - keyboardH - Self.toolViewHeight,
            width: view.eps.width,
            height: Self.toolViewHeight
        )
    ...
    @objc private func keyboardWillHide(_ notify: Notification) {
        ...
        let toolViewFrame = CGRect(
            x: 0,
            y: view.eps.height - deviceSafeAreaInsets().bottom - Self.toolViewHeight,
            width: view.eps.width,
            height: Self.toolViewHeight
        )
...
extension EPSInputTextViewController: UICollectionViewDelegate, UICollectionViewDataSource {
    func collectionView(_ collectionView: UICollectionView, numberOfItemsInSection section: Int) -> Int {
        return EPSPhotoConfiguration.default().editImageConfiguration.textStickerTextColors.count
    }
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        let c = EPSPhotoConfiguration.default().editImageConfiguration.textStickerTextColors[indexPath.row]
        cell.color = c
        if c == currentColor {
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        currentColor = EPSPhotoConfiguration.default().editImageConfiguration.textStickerTextColors[indexPath.row]
...
extension EPSInputTextViewController {
...
extension EPSInputTextViewController: UITextViewDelegate {
...
extension EPSInputTextViewController: NSLayoutManagerDelegate {
...
public enum ZLInputTextStyle {
    ...
    fileprivate var btnImage: UIImage? {
        switch self {
        case .normal:
            return .eps.getImage("zl_input_font")
        case .bg:
            return .eps.getImage("zl_input_font_bg")
        }
    }
}
