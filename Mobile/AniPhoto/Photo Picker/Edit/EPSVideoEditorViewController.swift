//
//  EPSVideoEditorViewController.swift
//  AniPhoto
//
//  Created by PhatCH on 2020/8/30.
//
...
public class EPSVideoEditorViewController: UIViewController {
    ...
    private lazy var cancelBtn: UIButton = {
        ...
        btn.setTitleColor(.eps.bottomToolViewBtnNormalTitleColor, for: .normal)
    ...
    private lazy var collectionView: UICollectionView = {
        ...
        layout.itemSize = EPSVideoEditorViewController.frameImageSize
        ...
        ZLEditVideoFrameImageCell.eps.register(view)
        return view
    }()
    ...
    private lazy var leftSideView: UIImageView = {
        let view = UIImageView(image: .eps.getImage("zl_ic_left"))
        view.isUserInteractionEnabled = true
        return view
    }()
    
    private lazy var rightSideView: UIImageView = {
        let view = UIImageView(image: .eps.getImage("zl_ic_right"))
        view.isUserInteractionEnabled = true
        return view
    }()
    ...
    private lazy var interval: TimeInterval = {
        ...
        return min(assetDuration, TimeInterval(EPSPhotoConfiguration.default().maxEditVideoTime)) / 10
    }()
    ...
    private lazy var generator: AVAssetImageGenerator = {
        ...
        g.maximumSize = CGSize(width: EPSVideoEditorViewController.frameImageSize.width * 3, height: EPSVideoEditorViewController.frameImageSize.height * 3)
    ...
    deinit {
        eps_debugPrint("EPSVideoEditorViewController deinit")
        cleanTimer()
    ...
    override public func viewDidLayoutSubviews() {
        ...
        eps_debugPrint("edit video layout subviews")
        ...
        let diffBottom = btnH + EPSVideoEditorViewController.frameImageSize.height + bottomBtnAndColSpacing + insets.bottom + 30
        ...
        let cancelBtnW = localLanguageTextValue(.cancel).eps.boundingRect(font: EPSLayout.bottomToolTitleFont, limitSize: CGSize(width: CGFloat.greatestFiniteMagnitude, height: btnH)).width
        ...
        collectionView.frame = CGRect(x: 0, y: doneBtn.frame.minY - bottomBtnAndColSpacing - EPSVideoEditorViewController.frameImageSize.height, width: view.bounds.width, height: EPSVideoEditorViewController.frameImageSize.height)
        
        let frameViewW = EPSVideoEditorViewController.frameImageSize.width * 10
        frameImageBorderView.frame = CGRect(x: (view.bounds.width - frameViewW) / 2, y: collectionView.frame.minY, width: frameViewW, height: EPSVideoEditorViewController.frameImageSize.height)
        // 左右拖动view
        let leftRightSideViewW = EPSVideoEditorViewController.frameImageSize.width / 2
        leftSideView.frame = CGRect(x: frameImageBorderView.frame.minX, y: collectionView.frame.minY, width: leftRightSideViewW, height: EPSVideoEditorViewController.frameImageSize.height)
        let rightSideViewX = view.bounds.width - frameImageBorderView.frame.minX - leftRightSideViewW
        rightSideView.frame = CGRect(x: rightSideViewX, y: collectionView.frame.minY, width: leftRightSideViewW, height: EPSVideoEditorViewController.frameImageSize.height)
    ...
    @objc private func doneBtnClick() {
        ...
        let d = CGFloat(interval) * clipRect().width / EPSVideoEditorViewController.frameImageSize.width
        if EPSPhotoConfiguration.Second(round(d)) < EPSPhotoConfiguration.default().minSelectVideoDuration {
            let message = String(format: localLanguageTextValue(.shorterThanMinVideoDuration), EPSPhotoConfiguration.default().minSelectVideoDuration)
            showAlertView(message, self)
            return
        }
        if EPSPhotoConfiguration.Second(round(d)) > EPSPhotoConfiguration.default().maxSelectVideoDuration {
            let message = String(format: localLanguageTextValue(.longerThanMaxVideoDuration), EPSPhotoConfiguration.default().maxSelectVideoDuration)
            showAlertView(message, self)
            return
        }
        ...
        EPSVideoManager.exportEditVideo(for: avAsset, range: getTimeRange()) { [weak self] url, error in
            hud.hide()
            if let er = error {
    ...
    private func startTimer() {
        cleanTimer()
        let duration = interval * TimeInterval(clipRect().width / EPSVideoEditorViewController.frameImageSize.width)
        
        timer = Timer.scheduledTimer(timeInterval: duration, target: EPSWeakProxy(target: self), selector: #selector(playPartVideo), userInfo: nil, repeats: true)
        timer?.fire()
        ...
        let indicatorH = leftSideView.eps.height
        let indicatorY = leftSideView.eps.top
        var indicatorFromX = leftSideView.eps.left
        var indicatorToX = rightSideView.eps.right - indicatorW
    ...
    private func getStartTime() -> CMTime {
        ...
        if measureCount > 10 {
            ...
            oneFrameDuration = (avAsset.duration.seconds - Double(EPSPhotoConfiguration.default().maxEditVideoTime)) / Double(measureCount - 10)
        }
        
        let offsetX = collectionView.contentOffset.x
        let previousSecond = offsetX / EPSVideoEditorViewController.frameImageSize.width * oneFrameDuration
        
        // 框选区域内起始时长
        let innerRect = frameImageBorderView.convert(clipRect(), from: view)
        let innerPreviousSecond: TimeInterval
        if isRTL() {
            innerPreviousSecond = (frameImageBorderView.eps.width - innerRect.maxX) / EPSVideoEditorViewController.frameImageSize.width * interval
        } else {
            innerPreviousSecond = innerRect.minX / EPSVideoEditorViewController.frameImageSize.width * interval
        }
    ...
    private func showFetchFailedAlert() {
        let action = EPSCustomAlertAction(title: localLanguageTextValue(.ok), style: .default) { [weak self] _ in
...
extension EPSVideoEditorViewController: UIGestureRecognizerDelegate {
...
extension EPSVideoEditorViewController: UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    ...
    public func collectionView(_ collectionView: UICollectionView, layout collectionViewLayout: UICollectionViewLayout, insetForSectionAt section: Int) -> UIEdgeInsets {
        let w = EPSVideoEditorViewController.frameImageSize.width * 10
    ...
    public func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        let cell = collectionView.dequeueReusableCell(withReuseIdentifier: ZLEditVideoFrameImageCell.eps.identifier, for: indexPath) as! ZLEditVideoFrameImageCell
