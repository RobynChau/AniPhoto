...
@objcMembers
public class EPSPhotoManager: NSObject {
    /// Save image to album.
    public class func saveImageToAlbum(image: UIImage, completion: ((Bool, PHAsset?) -> Void)?) {
        ...
        if image.eps.hasAlphaChannel(), let data = image.pngData() {
    ...
    public class func fetchPhoto(in result: PHFetchResult<PHAsset>, ascending: Bool, allowSelectImage: Bool, allowSelectVideo: Bool, limitCount: Int = .max) -> [EPSPhotoModel] {
        var models: [EPSPhotoModel] = []
    ...
    public class func getPhotoAlbumList(ascending: Bool, allowSelectImage: Bool, allowSelectVideo: Bool, completion: ([EPSAlbumListModel]) -> Void) {
        ...
        var albumList: [EPSAlbumListModel] = []
        arr.forEach { album in
            album.enumerateObjects { collection, _, _ in
                ...
                if collection.assetCollectionSubtype == .smartAlbumUserLibrary {
                    // Album of all photos.
                    let m = EPSAlbumListModel(title: title, result: result, collection: collection, option: option, isCameraRoll: true)
                    albumList.insert(m, at: 0)
                } else {
                    let m = EPSAlbumListModel(title: title, result: result, collection: collection, option: option, isCameraRoll: false)
                    albumList.append(m)
                }
            }
        }
    ...
    public class func getCameraRollAlbum(allowSelectImage: Bool, allowSelectVideo: Bool, completion: @escaping (EPSAlbumListModel) -> Void) {
        DispatchQueue.global().async {
            ...
            smartAlbums.enumerateObjects { collection, _, stop in
                if collection.assetCollectionSubtype == .smartAlbumUserLibrary {
                    ...
                    let result = PHAsset.fetchAssets(in: collection, options: option)
                    let albumModel = EPSAlbumListModel(title: self.getCollectionTitle(collection), result: result, collection: collection, option: option, isCameraRoll: true)
                    EPSMainAsync {
    ...
    public class func fetchVideo(for asset: PHAsset, progress: ((CGFloat, Error?, UnsafeMutablePointer<ObjCBool>, [AnyHashable: Any]?) -> Void)? = nil, completion: @escaping (AVPlayerItem?, [AnyHashable: Any]?, Bool) -> Void) -> PHImageRequestID {
        ...
        // https://github.com/longitachi/AniPhoto/issues/369#issuecomment-728679135
        if asset.eps.isInCloud {
    ...
    public class func fetchAssetSize(for asset: PHAsset) -> EPSPhotoConfiguration.KBUnit? {
    ...
    public class func saveAsset(_ asset: PHAsset, toFile fileUrl: URL, completion: @escaping ((Error?) -> Void)) {
        guard let resource = asset.eps.resource else {
        ...
        var timer: Timer? = .scheduledTimer(
            withTimeInterval: EPSPhotoUIConfiguration.default().timeout,
            repeats: false
        ) { timer in
            timer.invalidate()
            canceled = true
...
/// Authority related.
public extension EPSPhotoManager {
