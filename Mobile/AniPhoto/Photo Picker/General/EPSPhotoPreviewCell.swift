//
//  EPSPhotoPreviewCell.swift
//  AniPhoto
//
//  Created by PhatCH on 2020/8/21.
//
...
class EPSPreviewBaseCell: UICollectionViewCell {
    ...
    override init(frame: CGRect) {
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(previewVCScroll), name: EPSPhotoPreviewController.previewVCScrollNotification, object: nil)
    }
...
class EPSLocalImagePreviewCell: EPSPreviewBaseCell {
    ...
    deinit {
        eps_debugPrint("EPSLocalImagePreviewCell deinit")
    }
...
class EPSNetImagePreviewCell: EPSLocalImagePreviewCell {
...
class EPSPhotoPreviewCell: EPSPreviewBaseCell {
    override var currentImage: UIImage? { preview.image }
    ...
    deinit {
        eps_debugPrint("ZLPhotoPreviewCell deinit")
    }
...
class EPSGifPreviewCell: EPSPreviewBaseCell {
    override var currentImage: UIImage? { preview.image }
    ...
    deinit {
        eps_debugPrint("EPSGifPreviewCell deinit")
    }
...
class EPSLivePhotoPreviewCell: EPSPreviewBaseCell {
    ...
    deinit {
        eps_debugPrint("ZLLivePhotoPewviewCell deinit")
    }
    ...
    func loadLivePhotoData() {
        ...
        livePhotoRequestID = EPSPhotoManager.fetchLivePhoto(for: model.asset, completion: { livePhoto, _, isDegraded in
            if !isDegraded {
...
class EPSVideoPreviewCell: EPSPreviewBaseCell {
    override var currentImage: UIImage? {
        return imageView.image
    }
    ...
    private lazy var playBtn: UIButton = {
        ...
        btn.setImage(.eps.getImage("zl_playVideo"), for: .normal)
    ...
    private lazy var syncErrorLabel: UILabel = {
        ...
        attach.image = .eps.getImage("zl_videoLoadFailed")
        ...
        let errorText = NSAttributedString(
            ...
            attributes: [
                ...
                NSAttributedString.Key.font: UIFont.eps.font(ofSize: 12)
            ]
        )
    ...
    private let operationQueue = DispatchQueue(label: "com.AniPhoto.ZLVideoPreviewCell")
    ...
    deinit {
        ...
        eps_debugPrint("ZLVideoPreviewCell deinit")
    }
    ...
    private func configureCell() {
        ...
        resizeImageView(imageView: imageView, asset: model.asset)
        imageRequestID = EPSPhotoManager.fetchImage(for: model.asset, size: size, completion: { image, _ in
            self.imageView.image = image
        })
    }
    
    private func fetchVideo() {
        videoRequestID = EPSPhotoManager.fetchVideo(for: model.asset, progress: { [weak self] progress, _, _, _ in
            self?.progressView.progress = progress
            eps_debugPrint("video progress \(progress)")
            if progress >= 1 {
                eps_debugPrint("video load finished")
            ...
            let isFetchError = EPSPhotoManager.isFetchImageError(error)
            if isFetchError {
    ...
    private func configurePlayerLayer(_ item: AVPlayerItem) {
        playBtn.setImage(.eps.getImage("zl_playVideo"), for: .normal)
...
class EPSNetVideoPreviewCell: EPSPreviewBaseCell {
    private var player: AVPlayer?
    ...
    deinit {
        try? AVAudioSession.sharedInstance().setActive(false, options: .notifyOthersOnDeactivation)
        eps_debugPrint("EPSNetVideoPreviewCell deinit")
    }
...
class ZLPreviewView: UIView {
    ...
    private func loadPhoto() {
        if let editImage = model.editImage {
            ...
            imageRequestID = EPSPhotoManager.fetchImage(for: model.asset, size: requestPhotoSize(gif: false), progress: { [weak self] progress, _, _, _ in
                self?.progressView.progress = progress
                if progress >= 1 {
                    self?.progressView.isHidden = true
                } else {
    ...
    private func loadGifFirstFrame() {
        ...
        if EPSPhotoConfiguration.default().gifPlayBlock != nil {
            imageView.subviews.forEach { $0.removeFromSuperview() }
        }
        
        imageRequestID = EPSPhotoManager.fetchImage(for: model.asset, size: requestPhotoSize(gif: true), completion: { [weak self] image, _ in
            guard self?.imageIdentifier == self?.model.ident else {
                return
            }
    ...
    func loadGifData() {
        ...
        gifImageRequestID = EPSPhotoManager.fetchOriginalImageData(for: model.asset, progress: { [weak self] progress, _, _, _ in
            self?.progressView.progress = progress
            if progress >= 1 {
                self?.progressView.isHidden = true
            } else {
            ...
            if !isDegraded {
                ...
                if let gifPlayBlock = EPSPhotoConfiguration.default().gifPlayBlock {
                    gifPlayBlock(self.imageView, data, info)
                } else {
                    self.imageView.image = UIImage.eps.animateGifImage(data: data)
                }
    ...
    func resumeGif() {
        ...
        guard EPSPhotoConfiguration.default().allowSelectGif, m.type == .gif else { return }
