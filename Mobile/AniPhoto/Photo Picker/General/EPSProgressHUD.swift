//
//  EPSProgressHUD.swift
//  AniPhoto
//
//  Created by PhatCH on 2020/8/17.
//

import UIKit

public class EPSProgressHUD: UIView {
    private let style: EPSProgressHUD.Style
    ...
    deinit {
        eps_debugPrint("EPSProgressHUD deinit")
        cleanTimer()
    }
    
    public init(style: EPSProgressHUD.Style) {
        self.style = style
    ...
    public func show(
        toast: EPSProgressHUD.Toast = .loading,
        ...
        if timeout > 0 {
            cleanTimer()
            timer = Timer.scheduledTimer(timeInterval: timeout, target: EPSWeakProxy(target: self), selector: #selector(timeout(_:)), userInfo: nil, repeats: false)
...
public extension EPSProgressHUD {
    class func show(
        toast: EPSProgressHUD.Toast = .loading,
        in view: UIView? = UIApplication.shared.keyWindow,
        timeout: TimeInterval = 100
    ) -> EPSProgressHUD {
        let hud = EPSProgressHUD(style: EPSPhotoUIConfiguration.default().hudStyle)
    ...
    enum Style: Int {
        ...
        var icon: UIImage? {
            ...
            case .light, .lightBlur:
                return .eps.getImage("zl_loading_dark")
            case .dark, .darkBlur:
                return .eps.getImage("zl_loading_light")
            }
        }
