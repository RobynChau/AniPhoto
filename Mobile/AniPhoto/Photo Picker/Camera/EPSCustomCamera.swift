...
open class EPSCustomCamera: UIViewController {
    enum Layout {
    ...
    public lazy var smallCircleView: UIView = {
        ...
        view.layer.cornerRadius = EPSCustomCamera.Layout.smallCircleRadius / 2
        view.isUserInteractionEnabled = false
        view.backgroundColor = EPSCustomCamera.Layout.cameraBtnNormalColor
        return view
    }()
    
    public lazy var borderLayer: CAShapeLayer = {
        let animateLayerRadius = EPSCustomCamera.Layout.largeCircleRadius
        ...
        layer.strokeColor = EPSCustomCamera.Layout.cameraBtnNormalColor.cgColor
        layer.fillColor = UIColor.clear.cgColor
        layer.lineWidth = EPSCustomCamera.Layout.borderLayerWidth
        return layer
    }()
    ...
    public lazy var animateLayer: CAShapeLayer = {
        ...
        let layer = CAShapeLayer()
        layer.path = path.cgPath
        layer.strokeColor = UIColor.eps.cameraRecodeProgressColor.cgColor
        layer.fillColor = UIColor.clear.cgColor
        layer.lineWidth = EPSCustomCamera.Layout.animateLayerWidth
        layer.lineCap = .round
        return layer
    }()
    
    public lazy var retakeBtn: EPSEnlargeButton = {
        let btn = EPSEnlargeButton(type: .custom)
        btn.setImage(.eps.getImage("zl_retake"), for: .normal)
    ...
    public lazy var dismissBtn: EPSEnlargeButton = {
        let btn = EPSEnlargeButton(type: .custom)
        btn.setImage(.eps.getImage("zl_camera_close"), for: .normal)
    ...
    public lazy var flashBtn: EPSEnlargeButton = {
        let btn = EPSEnlargeButton(type: .custom)
        btn.setImage(.eps.getImage("zl_flash_off"), for: .normal)
        btn.setImage(.eps.getImage("zl_flash_on"), for: .selected)
    ...
    public lazy var switchCameraBtn: EPSEnlargeButton = {
        ...
        let btn = EPSEnlargeButton(type: .custom)
        btn.setImage(.eps.getImage("zl_toggle_camera"), for: .normal)
    ...
    public lazy var focusCursorView: UIImageView = {
        let view = UIImageView(image: .eps.getImage("zl_focus"))
    ...
    private let sessionQueue = DispatchQueue(label: "com.eps.camera.sessionQueue")
    ...
    private var cameraConfig : EPSCameraConfiguration!
    ...
    deinit {
        eps_debugPrint("ZLCustomCamera deinit")
        cleanTimer()
        try? AVAudioSession.sharedInstance().setActive(false, options: .notifyOthersOnDeactivation)
    }
    
    @objc public init() {
        cameraConfig = EPSPhotoConfiguration.default().cameraConfiguration
        canEdit = true
        super.init(nibName: nil, bundle: nil)
        modalPresentationStyle = .fullScreen
    }

    @objc public init(cameraConfig : EPSCameraConfiguration, canEdit : Bool) {
    ...
    override open func viewDidAppear(_ animated: Bool) {
        ...
        if !UIImagePickerController.isSourceTypeAvailable(.camera) {
            ...
            let animation = EPSAnimationUtils.animation(type: .fade, fromValue: 0, toValue: 1, duration: 0.15)
    ...
    override open func viewDidLayoutSubviews() {
        ...
        if isSmallScreen() {
            previewLayerY = deviceIsFringeScreen() ? min(94, view.eps.height - layerH) : 0
        } else {
            previewLayerY = 0
        }
        ...
        var bottomViewToBottomSpacing = view.eps.height - insets.bottom - EPSCustomCamera.Layout.bottomViewH
        if view.eps.height <= 812 {
            bottomViewToBottomSpacing -= deviceIsFringeScreen() ? 40 : 20
        }
        
        bottomView.frame = CGRect(x: 0, y: bottomViewToBottomSpacing, width: view.bounds.width, height: EPSCustomCamera.Layout.bottomViewH)
        let largeCircleH = EPSCustomCamera.Layout.largeCircleRadius
        largeCircleView.frame = CGRect(x: (view.bounds.width - largeCircleH) / 2, y: (EPSCustomCamera.Layout.bottomViewH - largeCircleH) / 2, width: largeCircleH, height: largeCircleH)
        let smallCircleH = EPSCustomCamera.Layout.smallCircleRadius
        smallCircleView.frame = CGRect(x: (view.bounds.width - smallCircleH) / 2, y: (EPSCustomCamera.Layout.bottomViewH - smallCircleH) / 2, width: smallCircleH, height: smallCircleH)
        
        flashBtn.frame = CGRect(x: 60, y: (EPSCustomCamera.Layout.bottomViewH - 25) / 2, width: 25, height: 25)
        switchCameraBtn.frame = CGRect(x: bottomView.eps.width - 60 - 25, y: flashBtn.eps.top, width: 25, height: 25)
        
        let tipsTextHeight = (tipsLabel.text ?? " ").eps
            .boundingRect(
                font: .eps.font(ofSize: 14),
        ...
        doneBtn.frame = CGRect(x: view.bounds.width - doneBtnW - 20, y: doneBtnY, width: doneBtnW, height: EPSLayout.bottomToolBtnH)
    }
    ...
    private func showNoMicrophoneAuthorityAlert() {
        let continueAction = EPSCustomAlertAction(title: localLanguageTextValue(.keepRecording), style: .default, handler: nil)
        let gotoSettingsAction = EPSCustomAlertAction(title: localLanguageTextValue(.gotoSettings), style: .tint) { _ in
    ...
    private func showAlertAndDismissAfterDoneAction(message: String, type: ZLNoAuthorityType?) {
        let action = EPSCustomAlertAction(title: localLanguageTextValue(.done), style: .default) { [weak self] _ in
            self?.dismiss(animated: true) {
                if let type = type {
                    if let customAlertWhenNoAuthority = EPSPhotoConfiguration.default().customAlertWhenNoAuthority {
                        customAlertWhenNoAuthority(type)
                    } else {
                        EPSPhotoConfiguration.default().noAuthorityCallback?(type)
                    }
                }
            }
        }
    ...
    private func startHideTipsLabelTimer() {
        cleanTimer()
        hideTipsTimer = Timer.scheduledTimer(timeInterval: 3, target: EPSWeakProxy(target: self), selector: #selector(hideTipsLabel_timerFunc), userInfo: nil, repeats: false)
    ...
    @objc private func switchCameraBtnClick() {
        ...
        sessionQueue.async {
            do {
                ...
                eps_debugPrint("切换摄像头失败 \(error.localizedDescription)")
            }
        }
    }
    ...
    @objc private func editImage() {
        ...
        EPSImageEditorViewController.showEditImageVC(parentVC: self, image: takedImage) { [weak self] in
            self?.retakeBtnClick()
...
//        self.recordVideoPlayerLayer?.player = nil
    ...
    @objc private func takePicture() {
        guard EPSPhotoManager.hasCameraAuthority(), !isTakingPicture else {
            return
        }
    ...
    @objc private func longPressAction(_ longGes: UILongPressGestureRecognizer) {
        if longGes.state == .began {
            guard EPSPhotoManager.hasCameraAuthority() else {
                return
            }
            startRecord()
    ...
    private func animateFocusCursor(point: CGPoint) {
        ...
        let scaleAnimation = EPSAnimationUtils.animation(type: .scale, fromValue: 2, toValue: 1, duration: 0.25)
        let fadeShowAnimation = EPSAnimationUtils.animation(type: .fade, fromValue: 0, toValue: 1, duration: 0.25)
        let fadeDismissAnimation = EPSAnimationUtils.animation(type: .fade, fromValue: 1, toValue: 0, duration: 0.25)
    ...
    private func setVideoZoomFactor(_ zoomFactor: CGFloat) {
        ...
        do {
            ...
            eps_debugPrint("调整焦距失败 \(error.localizedDescription)")
        }
    }
    ...
    private func focusCamera(mode: AVCaptureDevice.FocusMode, exposureMode: AVCaptureDevice.ExposureMode, point: CGPoint) {
        do {
            ...
            eps_debugPrint("相机聚焦设置失败 \(error.localizedDescription)")
        }
    }
    ...
    private func openTorch() {
        ...
        sessionQueue.async {
            do {
                ...
                eps_debugPrint("打开手电筒失败 \(error.localizedDescription)")
            }
        }
    }
    ...
    private func closeTorch() {
        ...
        sessionQueue.async {
            do {
                ...
                eps_debugPrint("关闭手电筒失败 \(error.localizedDescription)")
            }
        }
    }
    ...
    private func startRecord() {
        ...
        let url = URL(fileURLWithPath: EPSVideoManager.getVideoExportFilePath())
    ...
    private func startRecordAnimation() {
        UIView.animate(withDuration: 0.1, animations: {
            self.largeCircleView.layer.transform = CATransform3DScale(CATransform3DIdentity, EPSCustomCamera.Layout.largeCircleRecordScale, EPSCustomCamera.Layout.largeCircleRecordScale, 1)
            self.smallCircleView.layer.transform = CATransform3DScale(CATransform3DIdentity, EPSCustomCamera.Layout.smallCircleRecordScale, EPSCustomCamera.Layout.smallCircleRecordScale, 1)
            self.borderLayer.strokeColor = EPSCustomCamera.Layout.cameraBtnRecodingBorderColor.cgColor
            self.borderLayer.lineWidth = EPSCustomCamera.Layout.animateLayerWidth
        }) { _ in
    ...
    private func stopRecordAnimation() {
        EPSMainAsync {
            self.borderLayer.strokeColor = EPSCustomCamera.Layout.cameraBtnNormalColor.cgColor
            self.borderLayer.lineWidth = EPSCustomCamera.Layout.borderLayerWidth
...
extension EPSCustomCamera: AVCapturePhotoCaptureDelegate {
    public func photoOutput(_ output: AVCapturePhotoOutput, willCapturePhotoFor resolvedSettings: AVCaptureResolvedPhotoSettings) {
        EPSMainAsync {
            let animation = EPSAnimationUtils.animation(type: .fade, fromValue: 0, toValue: 1, duration: 0.25)
    ...
    public func photoOutput(_ output: AVCapturePhotoOutput, didFinishProcessingPhoto photoSampleBuffer: CMSampleBuffer?, previewPhoto previewPhotoSampleBuffer: CMSampleBuffer?, resolvedSettings: AVCaptureResolvedPhotoSettings, bracketSettings: AVCaptureBracketedStillImageSettings?, error: Error?) {
        EPSMainAsync {
            ...
            if photoSampleBuffer == nil || error != nil {
                eps_debugPrint("拍照失败 \(error?.localizedDescription ?? "")")
                return
            }
            ...
            if let data = AVCapturePhotoOutput.jpegPhotoDataRepresentation(forJPEGSampleBuffer: photoSampleBuffer!, previewPhotoSampleBuffer: previewPhotoSampleBuffer) {
                ...
                self.takedImage = UIImage(data: data)?.eps.fixOrientation()
                self.takedImageView.image = self.takedImage
                self.takedImageView.isHidden = false
                self.editImage()
            } else {
                eps_debugPrint("拍照失败，data为空")
            }
        }
    }
}

extension EPSCustomCamera: AVCaptureFileOutputRecordingDelegate {
    ...
    private func finishRecordAndMergeVideo() {
        EPSMainAsync {
            ...
            if self.recordURLs.count > 1 {
                ...
                EPSVideoManager.mergeVideos(fileURLs: self.recordURLs) { [weak self] url, error in
                    hud.hide()
...
extension EPSCustomCamera: CAAnimationDelegate {
...
extension EPSCustomCamera: UIGestureRecognizerDelegate {
