//
//  EPSPhotoPreviewPopInteractiveTransition.swift
//  AniPhoto
//
...
class EPSPhotoPreviewPopInteractiveTransition: UIPercentDrivenInteractiveTransition {
    weak var transitionContext: UIViewControllerContextTransitioning?
    
    weak var viewController: EPSPhotoPreviewController?
    ...
    var currentCell: EPSPreviewBaseCell?
    /// 取消动画时候，是否需要将Y值修正为0
    ...
    deinit {
        eps_debugPrint("EPSPhotoPreviewPopInteractiveTransition deinit")
    }
    
    init(viewController: EPSPhotoPreviewController) {
...
//            imageView?.frame = result.frame
    ...
    func canStartPan() -> Bool {
        ...
        if scrollView.isZooming ||
            ...
            (convertRect.minX != 0 && contentView.eps.width > scrollView.eps.width) {
            return false
        }
    ...
    func startAnimate() {
        ...
        guard let fromVC = transitionContext.viewController(forKey: .from) as? EPSPhotoPreviewController,
              let toVC = transitionContext.viewController(forKey: .to) as? EPSThumbnailViewController else {
            return
        }
        ...
        guard let cell = fromVC.collectionView.cellForItem(at: IndexPath(row: fromVC.currentIndex, section: 0)) as? EPSPreviewBaseCell else {
            return
        }
        
        currentCell = cell
        shadowView = UIView(frame: containerView.bounds)
        shadowView?.backgroundColor = EPSPhotoUIConfiguration.default().previewVCBgColor
        ...
        if let videoCell = cell as? EPSVideoPreviewCell, let playerLayer = videoCell.playerLayer, videoCell.imageView.isHidden {
    ...
    func cancelAnimate() {
        ...
        UIView.animate(withDuration: 0.25, animations: {
            ...
            if let playerLayer = self.playerLayer {
                playerLayer.removeFromSuperlayer()
                (self.currentCell as? EPSVideoPreviewCell)?.playerView.layer.insertSublayer(playerLayer, at: 0)
            }
    ...
    func resetViewStatus(isStart: Bool) {
        ...
        (currentCell as? EPSVideoPreviewCell)?.singleTapGes.isEnabled = !isStart
        
        guard let transitionContext = transitionContext,
              let fromVC = transitionContext.viewController(forKey: .from) as? EPSPhotoPreviewController else {
            return
        }
        
        fromVC.view.backgroundColor = isStart ? .clear : EPSPhotoUIConfiguration.default().previewVCBgColor
        fromVC.collectionView.isHidden = isStart
    }
}

extension EPSPhotoPreviewPopInteractiveTransition: UIGestureRecognizerDelegate {
    func gestureRecognizerShouldBegin(_ gestureRecognizer: UIGestureRecognizer) -> Bool {
        ...
        if shouldBegin,
           ...
           let scrollView = cell.scrollView {
            ...
            needCorrectYToZeroWhenCancel = contentSizeH > scrollView.eps.height && scrollView.contentOffset.y >= 0
        }
